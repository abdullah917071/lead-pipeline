...
def parse_function_arguments(raw_arguments: str) -> dict[str, Any]:
    """Parse standard JSON and tolerate an identical duplicated stream fragment."""
    try:
        return json.loads(raw_arguments)
    except json.JSONDecodeError as error:
        value, end = json.JSONDecoder().raw_decode(raw_arguments)
        first = raw_arguments[:end]
        remainder = raw_arguments[end:].strip()
        if remainder == first:
            return value
        raise error
        ...
        async with _closing(chunk_stream) as chunk_iter:
            async for chunk in chunk_iter:
                ...
                if chunk.choices[0].delta.tool_calls:
                    ...
                    # Sarvam can emit whitespace alongside a tool-only response.
                    # No TTS frame is then produced, so deferring a transition until
                    # BotStoppedSpeakingFrame deadlocks the call. Only defer when
                    # actual speakable text was emitted.
