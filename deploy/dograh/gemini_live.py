...
class DograhGeminiLiveLLMService(GeminiLiveLLMService):
    ...
    async def _send_user_audio(self, frame):
        if self._user_is_muted:
            return
        # Telnyx provides 8 kHz PCM. Gemini Live native-audio recognition
        # requires 16 kHz PCM; 8 kHz made local VAD fire but Gemini never
        # completed the caller turn. Resample each packet before Gemini sees it
        # so each Telnyx media packet is delivered to Gemini immediately.
        if (
        ...
        if not self._vad_disabled or self._user_is_speaking:
            ...
            if sample_rate != 16000:
                resampler = getattr(self, "_gemini_input_resampler", None)
                if resampler is None:
                    resampler = SOXRAudioResampler()
                    self._gemini_input_resampler = resampler
                audio = await resampler.resample(audio, sample_rate, 16000)
                sample_rate = 16000
            try:
        ...
        # Retain upstream pre-roll behaviour for locally-driven VAD.
