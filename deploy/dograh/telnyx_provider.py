...
class TelnyxProvider(TelephonyProvider):
    ...
    async def validate_phone_number(self, address: str) -> ProviderSyncResult:
        """Verify the caller ID belongs to this Telnyx account."""
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.TELNYX_API_BASE}/phone_numbers",
                    params={"filter[phone_number]": normalized.canonical},
                    headers=self._headers(),
                ) as response:
                    if response.status != 200:
                        body = await response.text()
                        raise ProviderPhoneNumberLookupError(
                            f"Telnyx phone-number lookup returned HTTP {response.status}: {body}"
                        )
            ...
            raise ProviderPhoneNumberLookupError(
                f"Telnyx phone-number lookup failed: {exc}"
            ) from exc

        for number in data.get("data") or []:
            if number.get("phone_number") == normalized.canonical:
                return ProviderSyncResult(ok=True)
        return ProviderSyncResult(
            ok=False,
            message=f"Phone number {normalized.canonical} is not owned by this Telnyx account.",
        )
    ...
    async def handle_websocket(
        ...
        try:
            ...
            # pinned to PCMA separately via stream_bidirectional_codec.
            try:
